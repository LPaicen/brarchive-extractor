...
`exist.json` and `contents.json` are optional. `exist.json` provides export-version metadata when available. If it is absent, unreadable, or has no string `version`, the console summary displays `unknown version`. `contents.json` is not needed for decoding.
